...
A snakemake pipeline that blasts your fasta files against a given genome in NCBI.
...
queries = glob_wildcards(query_folder_path + "{id}.fq.gz") #use \w+ wldcard constraint ({id,\w+}) to prevent subfolders being searched. Remember, a wildcard is any string, including a folder path
print(queries)
